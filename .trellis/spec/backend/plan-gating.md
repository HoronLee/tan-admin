...
- `src/lib/plan.ts` — 枚举、limits 表、helpers
- `src/lib/auth.ts` `teams.maximumTeams` / `organizationHooks.beforeCreateInvitation` / 任何读 `organization.plan` 的位置
...
核心原则：**plan → limits 的映射只在 `src/lib/plan.ts` 里，别处只消费 helper**。新加 plan 或改 limits 只改一个文件，全项目自动同步。
