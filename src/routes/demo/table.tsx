...
function TableDemo() {
	...
	const { columnFilters: activeColumnFilters, sorting } = table.getState();

	// apply the fuzzy sort if the fullName column is being filtered
	React.useEffect(() => {
		const firstFilter = activeColumnFilters[0];
		const firstSort = sorting[0];
		if (firstFilter?.id === "fullName" && firstSort?.id !== "fullName") {
			table.setSorting([{ id: "fullName", desc: false }]);
		}
	}, [activeColumnFilters, sorting, table]);
