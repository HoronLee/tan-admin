...
// BA configuration without `database`. server.ts and codegen.ts each spread
// this and bind their own adapter (pg.Pool for runtime, prismaAdapter for
// schema generation). Keeping config in one place ensures the generated
// shadow schema can never drift from the runtime instance.
