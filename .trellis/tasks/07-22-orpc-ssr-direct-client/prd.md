...
- [ ] SSR 预取 `getUserMenus` 时走 in-process router client，不产生发往本机 `/api/rpc` 的 fetch。
- [ ] 每个 SSR 请求看到自己的 headers/session，不发生跨请求身份串线。
- [ ] 浏览器端仍通过 `/api/rpc` 正常调用，且 `src/orpc/client.ts` 没有 runtime router import。
- [ ] 缺少 server registration 时 SSR 测试得到明确失败，而不是依赖 `SERVER_URL`/host fallback。
- [ ] `pnpm check`、`pnpm test`、`pnpm build` 通过，build 不出现新的 server-only/client import 警告。
