...
### 添加 MCP tool
在 [src/routes/mcp.ts](src/routes/mcp.ts) 的 `server.registerTool` 加新项。需要数据库访问就 import `db`。

## Sentry 装载与埋点

- `instrument.server.mjs` 在 **进程启动前**被 `NODE_OPTIONS='--import ./instrument.server.mjs'` 预加载，初始化 Sentry SDK。`pnpm dev` 和 `pnpm start` 都带这个 flag，`pnpm build` 会把它复制到 `.output/server/`。
- 前端错误收集配置在 `src/router.tsx`，自动工作。
- 新增 **server function**（`createServerFn`）时，手动用 `Sentry.startSpan` 包裹耗时操作：

```ts
import * as Sentry from '@sentry/tanstackstart-react'

const loadData = createServerFn({ method: 'GET' }).handler(async () => {
  return Sentry.startSpan({ name: 'loadData' }, async () => {
...
- 残留的 `ClAUDE.md`（小写 L）符号链接可删：`rm ClAUDE.md`，保留标准 `CLAUDE.md`。
- `src/generated/prisma/` 和 `src/paraglide/`、`src/routeTree.gen.ts` 都是**生成产物**，勿手改，勿提交到 git（已在 `.gitignore`）。
- Prisma Client 路径走 `output = "../src/generated/prisma"`（见 `prisma/schema.prisma`），改路径记得同步 import。
